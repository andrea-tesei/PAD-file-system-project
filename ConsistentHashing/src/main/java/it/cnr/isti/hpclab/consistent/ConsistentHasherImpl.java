...
{
	/**
	 * The value 700 for virtual instances provides better distribution among the buckets.
	 */
	 ...
	 */
	public ConsistentHasherImpl(final int virtualInstancesPerBucket,
								final BytesConverter<B> bucketDataToBytesConverter,
								final BytesConverter<M> memberDataToBytesConverter,
								final HashFunction hashFunction) 
	{
		Preconditions.checkNotNull(bucketDataToBytesConverter, "Bucket data converter can not be null.");
		Preconditions.checkNotNull(memberDataToBytesConverter, "Member data converter can not be null.");
		Preconditions.checkNotNull(hashFunction,			   "HashFunction can not be null.");
		...
		this.stopVirtualNodeId = (virtualInstancesPerBucket > 0) ? virtualInstancesPerBucket : 1;
	...
	{
		byte[] bucketNameInBytes = bucketDataToBytesConverter.convert(bucketName);
	...
	public void addBucket(B bucketName) 
	{
		...
		for (int virtualNodeId = startVirtualNodeId; virtualNodeId <= stopVirtualNodeId; virtualNodeId++) {
			ByteBuffer virtBucket = convertAndApplyHash(virtualNodeId, bucketName);
	...
	{
		return ByteBuffer.wrap(hashFunction.hash(memberDataToBytesConverter.convert(memberName)));
	}
	...
	private List<M> getMembersInternal(B bucketName, NavigableMap<ByteBuffer, M> members) 
	{
			...
			List<? extends M> members) 
	{
		...
		for (int virtNodeId = startVirtNodeId; virtNodeId <= endVirtNodeId; virtNodeId++) {
			ConsistentHasher<B, M> cHasher = new ConsistentHasherImpl<>(
					...
					hashFunction);
			buckets.stream().forEach(bucketName -> cHasher.addBucket(bucketName));
