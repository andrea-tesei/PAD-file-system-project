			...
			if (gossipManager.getMemberList().contains(remoteMember)) {
				...
				if (gossipManager.getDeadList().contains(remoteMember)) {
					...
					if(localDeadMember.getLastKnownHeartbeat() != remoteMember.getHeartbeat()){
						if (((localDeadMember.getLastKnownHeartbeat() - remoteMember.getHeartbeat()) * -1) > ((gossipManager.getSettings().getCleanupInterval() / 1000)) || remoteMember.getHeartbeat() > localDeadMember.getHeartbeat()){ /* || remoteMember.getHeartbeat() == 1 ||  */  
