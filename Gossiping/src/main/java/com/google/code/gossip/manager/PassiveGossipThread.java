...
abstract public class PassiveGossipThread implements Runnable 
{
	...
	public PassiveGossipThread(final GossipManager gossipManager) 
	{
		...
		try {
			SocketAddress socketAddress = new InetSocketAddress(gossipManager.getMyself().getHost(), gossipManager.getMyself().getPort());
			udpServer = new DatagramSocket(socketAddress);
			GossipService.LOGGER.info("Gossip service successfully initialized on port " + gossipManager.getMyself().getPort());
					...
					try {
						...
						for (int i = 0; i < jsonArray.length(); i++) {
							...
							if (memberJSONObject.length() == 4) {
								...
								GossipService.LOGGER.error("The received member object does not contain 4 objects:\n" + memberJSONObject.toString());
							}
						}
   ...
   */
  abstract protected void mergeLists(final GossipManager gossipManager, final RemoteGossipMember senderMember, final List<GossipMember> remoteList);
}
